...
function renderTriangles()
{
	...
	for(i; i > -1; --i)
	{
		...
		for(j; j > -1; --j)
		{
			...
			context.beginPath();
			// console.log(quad.getPos().getX() + Math.cos(p1.getAngle().getX()) * p1.getSize() +','+ quad.getPos().getY() + Math.sin(p1.getAngle().getY()) * p1.getSize());
			...
			// context.rect(quad.getPos().getX() + Math.cos(p1.getAngle().getX()) * p1.getSize() - 4, quad.getPos().getY() + Math.sin(p1.getAngle().getY()) * p1.getSize() - 4,8,8);
			context.fill();
			context.stroke();
			context.closePath();
		}
	...
	create: function(x, y, size)
	{
		...
		(
			[
				...
				point.create(size, Math.PI * 0.25, Math.PI * 0.25),
				point.create(size, Math.PI * 0.5, Math.PI * 0.5),
				point.create(size, Math.PI * 0.75, Math.PI * 0.75),
				point.create(size, Math.PI, Math.PI),
				point.create(size, Math.PI * 1.25, Math.PI * 1.25),
				point.create(size, Math.PI * 1.5, Math.PI * 1.5),
				point.create(size, Math.PI * 1.75, Math.PI * 1.75)
			]
		);
